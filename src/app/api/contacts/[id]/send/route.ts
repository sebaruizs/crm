...
interface SendBody {
  text?: string;
  media?: {
    fileId: string;     // returned by /api/files/upload (id with extension)
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!text && !media) {
    return NextResponse.json({ error: "text o media requerido" }, { status: 400 });
  }
  ...
  let result: Awaited<ReturnType<typeof baileys.send>>;
